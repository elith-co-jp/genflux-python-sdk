...
<!-- Generated at: 2026-03-16 06:26 UTC -->
...
| 属性 | 型 | 説明 |
|---|---|---|
| `api_key` | `str \| None` | APIキー。指定しない場合は、環境変数 GENFLUX_API_KEY を使用します。 |
| `base_url` | `str \| None` | GenFlux API のベースURL。指定しない場合は、環境変数 GENFLUX_API_BASE_URL を使用するか、環境設定から構築されます。 |
| `environment` | `str \| None` | 環境名 ("local", "dev", "prod")。指定しない場合は、環境変数 GENFLUX_ENVIRONMENT を使用し、デフォルトは "prod" です。 |
| `timeout` | `int` | リクエストのタイムアウト時間（秒単位）。デフォルトは60秒です。 |

#### コンストラクタパラメータ

| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `api_key` | `str \| None` | No | APIキー。認証に使用されます。指定しない場合は、環境変数 GENFLUX_API_KEY を使用します。 |
| `base_url` | `str \| None` | No | GenFlux API のベースURL。指定しない場合は、環境変数 GENFLUX_API_BASE_URL を使用するか、環境設定から構築されます。 |
| `environment` | `str \| None` | No | 環境名 ("local", "dev", "prod")。指定しない場合は、環境変数 GENFLUX_ENVIRONMENT を使用し、デフォルトは "prod" です。 |
| `timeout` | `int` | No | リクエストのタイムアウト時間（秒単位）。デフォルトは60秒です。 |
...
| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `config` | `ConfigCreate` | **Yes** | コンフィグ作成のためのパラメータ |
...
|---|---|---|---|
| `config_id` | `str \| UUID` | **Yes** | コンフィグID |
| `config_update` | `ConfigUpdate` | **Yes** | コンフィグ更新のためのパラメータ |
...
| `config_id` | `str \| None` | No | コンフィグID（省略可能、指定しない場合はデフォルトを使用） |
...
- `APIError`: APIリクエストが失敗した場合
- `ValidationError`: リクエストの検証に失敗した場合
...
| `timeout` | `int` | No | 最大待機時間（秒単位）。デフォルトは600秒です。 |
| `poll_interval` | `float` | No | ポーリング間隔（秒単位）。デフォルトは5.0秒です。 |
| `callback` | `Optional[Callable[[Job], None]]` | No | 各ポーリング時にJobオブジェクトと共に呼び出されるオプションのコールバック関数 |
...
| `answer` | `str` | **Yes** | 答えのテキスト |
| `contexts` | `list[str] \| None` | No | コンテキスト/リトリーバルテキスト（オプション） |
| `timeout` | `int` | No | 最大待機時間（秒単位） |
...
| `question` | `str` | **Yes** | 質問のテキスト |
| `answer` | `str` | **Yes** | 答えのテキスト |
| `contexts` | `list[str]` | **Yes** | コンテキスト/リトリーバルテキスト（順序が重要） |
| `timeout` | `int` | No | 最大待機時間（秒単位） |
...
| `question` | `str` | **Yes** | 質問のテキスト |
| `answer` | `str` | **Yes** | 答えのテキスト |
| `contexts` | `list[str]` | **Yes** | コンテキスト/リトリーバルテキスト |
| `ground_truth` | `str` | **Yes** | コンテキストリコールに必要な正解の答え |
| `timeout` | `int` | No | 最大待機時間（秒単位） |
...
| `answer` | `str` | **Yes** | 答えのテキスト |
| `contexts` | `list[str] \| None` | No | コンテキスト/リトリーバルテキスト（オプション） |
| `ground_truth` | `str \| None` | No | コンテキストリコールに必要な正解の答え |
| `timeout` | `int` | No | 最大待機時間（秒単位）。デフォルトは300秒です。 |
...
| `policy_compliance_rate_threshold` | `float \| None` | コンプライアンス率閾値（%）です。 |
...
| `APIError` | `GenFluxError` | — | API リクエスト失敗（基底）です。 |
...
| `JobFailedError` | `GenFluxError` | — | ジョブ実行に失敗しました。 |
...
try:
    ...
    # レート制限です。e.retry_after 秒後にリトライします。
    pass
except TimeoutError:
    # ジョブがタイムアウトしました。
    pass
except JobFailedError as e:
    # ジョブ実行に失敗しました。e.error_message で詳細を確認します。
    pass
```
...
| `decimals` | `int` | パーセンテージ表示の小数点以下の桁数です (デフォルト: 1)。 |
| `fill` | `str` | プログレスバーの進行部分を表す文字です (デフォルト: '█')。 |
| `print_end` | `str` | プログレスバーの終了時に使用する文字列です (デフォルト: '\r')。 |
| `file` | `TextIO` | 出力先のファイルオブジェクトです (デフォルト: `<_io.TextIOWrapper name='<stdout>' mode='w' encoding='utf-8'>`)。 |
...
*Auto-generated at 2026-03-16 06:26 UTC by `scripts/generate_api_reference.py`*
