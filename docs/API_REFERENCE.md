...
<!-- Generated at: 2026-03-12 12:21 UTC -->
...
GenFlux API Client is used to interact with the GenFlux API, providing methods for evaluation and configuration management.
...
| `environment` | `str \| None` | 環境名（"local"、"dev"、または "prod"）。指定されない場合は環境変数 `GENFLUX_ENVIRONMENT` を使用します。デフォルトは "prod" です。 |
...
| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `api_key` | `str \| None` | No | 認証用のAPIキー。指定されない場合は `GENFLUX_API_KEY` 環境変数を使用します。 |
| `base_url` | `str \| None` | No | GenFlux APIのベースURL。指定されない場合は `GENFLUX_API_BASE_URL` 環境変数を使用するか、環境設定から構築します。 |
| `environment` | `str \| None` | No | 環境名（"local"、"dev"、または "prod"）。指定されない場合は `GENFLUX_ENVIRONMENT` 環境変数を使用します。デフォルトは "prod" です。 |
...
指定された設定の評価クライアントを作成します。

**パラメータ:**

| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `config_id` | `str \| None` | No | 評価に使用する設定ID（オプション、指定されない場合はデフォルトを使用） |

**戻り値:** EvaluationClient インスタンス
...
評価設定を管理するためのクライアントで、設定の作成、削除、取得、更新を行うことができます。
...
ジョブ（実行）管理のためのクライアントで、ジョブの作成、キャンセル、監視を行う機能を提供します。
...
| `config_id` | `str \| None` | No | 設定ID（オプション、指定されない場合はデフォルトを使用） |
...
| `callback` | `Optional[Callable[[Job], None]]` | No | 各ポールで呼び出されるオプションのコールバック関数 |
...
レポートAPIのクライアントで、さまざまな詳細レベルのレポートを取得するためのメソッドを提供します。
...
評価操作のためのクライアントで、ジョブベースの非同期実行を使用した同期スタイルのインターフェースを提供します。
...
**戻り値:** バイアススコアを持つMetricResult（低いほど良い）
...
このメソッドは、内部的にジョブを作成し、完了を待ち、結果を返す同期スタイルのAPIを提供します。
...
| `show_progress` | `bool` | No | 進捗バーを表示（デフォルト：True、callbackが提供された場合は無視されます） |

**戻り値:** スコアと理由を持つMetricResult
...
**戻り値:** 幻覚スコアを持つMetricResult（低いほど良い）
...
**戻り値:** 毒性スコアを持つMetricResult（低いほど良い）
...
| 属性 | 型 | 説明 |
|---|---|---|
| `id` | `UUID` | コンフィグの一意の識別子です。 |
| `tenant_id` | `UUID` | テナントの一意の識別子です。 |
| `user_id` | `UUID` | ユーザーの一意の識別子です。 |
| `name` | `str` | コンフィグの名前です。 |
| `description` | `str \| None` | コンフィグの説明です。 |
| `locale` | `str` | ロケール情報です。 |
| `api_settings` | `ApiSettings \| None` | API設定情報です。 |
| `rag_quality_config` | `RagQualityConfig \| None` | RAG品質評価の設定です。 |
| `redteam_config` | `RedteamConfig \| None` | RedTeam評価の設定です。 |
| `policy_check_config` | `PolicyCheckConfig \| None` | ポリシーチェックの設定です。 |
...
|---|---|---|
| `id` | `str` | ジョブの一意の識別子です。 |
| `tenant_id` | `str` | テナントの一意の識別子です。 |
| `user_id` | `str` | ユーザーの一意の識別子です。 |
| `config_id` | `str` | 使用するコンフィグのIDです。 |
...
|---|---|---|---|
| `cls` |  | **Yes** | クラス自身です。 |
| `data` | `dict[str, Any]` | **Yes** | APIレスポンスの辞書データです。 |
...
| `config_id` | `UUID \| None` | コンフィグの一意の識別子です。 |
| `type` | `str` | レポートのタイプです。 |
| `status` | `Literal[completed, partial]` | レポートのステータスです。 |
| `created_at` | `datetime` | 作成日時です。 |
| `summary` | `ReportSummary` | レポートのサマリです。 |
| `details` | `ReportDetails \| None` | レポートの詳細情報です。 |
...
| `passed` | `int` | 合格したテスト数です。 |
| `failed` | `int` | 失敗したテスト数です。 |
...
| `reason` | `str` | 失敗理由です。 |
| `category` | `str` | カテゴリ名です。 |
| `severity` | `Literal[low, medium, high, critical]` | 深刻度です。 |
...
|---|---|---|---|
| `GenFluxError` | `Exception` | — | 基底例外クラスです。 |
| `APIError` | `GenFluxError` | — | API リクエスト失敗の基底例外です。 |
...
try:
    ...
    # レート制限です。e.retry_after 秒後にリトライしてください。
    pass
except TimeoutError:
    # ジョブがタイムアウトしました。
    pass
...
| `print_end` | `str` | 出力の終了文字（デフォルト: '\r'） |
| `file` | `TextIO` | 出力ストリーム（デフォルト: <_io.TextIOWrapper name='<stdout>' mode='w' encoding='utf-8'>） |
...
| `indeterminate` | `bool` | No | Trueの場合、パーセンテージの代わりに「Processing...」を表示します（単一メトリックまたは初期状態向け） |
...
*Auto-generated at 2026-03-12 12:21 UTC by `scripts/generate_api_reference.py`*