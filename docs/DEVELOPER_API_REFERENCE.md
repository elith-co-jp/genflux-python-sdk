...
<!-- Generated at: 2026-09-26 07:58 UTC -->
...
#### `get_assessment_revision(job_id: str, assessment_id: str, revision: int) -> AssessmentBundle`
...
#### `get_by_client_request(client_request_id: str) -> Job`
...
**戻り値:** MetricResult with hallucination score (higher is better)
...
| `usage_summary` | `ExecutionUsageSummary \| None` | (default: None) |
| `client_request_id` | `str \| None` | (default: None) |
...
| パラメータ | 型 | 必須 | 説明 |
|---|---|---|---|
| `metric` | `str` | **Yes** | 評価メトリック名（例: "faithfulness", "answer_relevancy"）。 |
| `score` | `float` | **Yes** | 評価スコア（0.0〜1.0）。 |
| `reason` | `str \| None` | **Yes** | 評価理由の説明文。メトリックによっては None。 |
| `engine` | `str` | **Yes** | 評価エンジン識別子。現在は ``"genflux"`` が返される。 |
| `execution_time_seconds` | `float \| None` | No | 評価の実行時間（秒）。取得できない場合は None。 |
...
| `model` | `str` |  |
| `stage` | `str` |  |
| `measurement` | `Literal[actual, estimated, unavailable]` |  |
| `billable` | `bool` |  |
| `tokens` | `TokenCounts` |  |
...
| `measurement` | `Literal[actual, estimated, unavailable]` |  |
| `totals` | `TokenCounts` |  |
| `breakdown` | `list[ProviderTokenUsage]` |  |
...
| `tokens` | `TokenUsageSummary \| None` |  |
| `credits` | `CreditUsageSummary \| None` |  |
...
| `usage_summary` | `ExecutionUsageSummary \| None` |  |
| `assessment_bundle` | `AssessmentBundle \| None` |  |
| `accepted_assessment_plan` | `AcceptedAssessmentPlan \| None` |  |

#### メソッド

#### *classmethod* `validate_accepted_plan(cls, value: object) -> AcceptedAssessmentPlan | None`
...
#### *classmethod* `validate_assessment_bundle(cls, value: object) -> AssessmentBundle | None`
...
#### `validate_assessment_scope() -> Report`
...
*Auto-generated at 2026-09-26 07:58 UTC by `scripts/generate_api_reference.py`*

<!-- source-hash: f05366b9f63d00ed -->
